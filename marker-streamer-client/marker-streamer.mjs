...
class MarkerStreamer {
    ...
    getMarker( entityId ) {
        let marker = this.markers.find( m => +m.entityId === +entityId );
    ...
    restoreMarker( entityId ) {
        let marker = this.getMarker( entityId );
    ...
    clearMarker( entityId ) {
        let idx = this.markers.findIndex( m => +m.entityId === +entityId );
...
alt.on( "resourceStop", ( ) => {
    markerStreamer.markers.forEach( ( marker ) => {
        markerStreamer.removeMarker( +marker.entityId );
        markerStreamer.clearMarker( +marker.entityId );
    } );
} );
